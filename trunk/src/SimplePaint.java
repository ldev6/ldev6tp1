	...
	public void draw( GraphicsWrapper gw, boolean isFilled, boolean isHilited ) {
		...
		switch (type) {
			case POLYGON: {
				...
				System.out.println("merde "+points.size());
			} break;
			case CIRCLE: {
...
class MyCanvas extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void paintComponent( Graphics g ) {
		...
		if ( is2ndPointBeingDraggedOut || is3rdPointBeingDraggedOut || isBoxSelection) {
		...
		if ( SwingUtilities.isLeftMouseButton(e) ) {
			if ( simplePaint.currentTool == SimplePaint.TOOL_SELECT_AND_MOVE ) {
				if ( currentlyHilitedShape > -1 ) {
					...
					System.out.println("Je click sur le bouton");
					isBoxSelection = true;
					x1 = x2 = x3 = mouse_x;
					y1 = y2 = y3 = mouse_y;
		...
		if ( simplePaint.currentTool == SimplePaint.TOOL_SELECT_AND_MOVE ) {
			if ( SwingUtilities.isLeftMouseButton(e) ) {
				...
				if(isBoxSelection){
					...
					repaint();
					System.out.println("Je lache le bouton");
				}
