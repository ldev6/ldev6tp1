...
class MyShape {
    ...
    public boolean currentlyHilited = false;
...
class MyCanvas extends JPanel implements MouseListener, MouseMotionListener {
	...
	//This is used in Select and Move mode to create a boxselection
	int isBoxSelection = 0;
	...
	public void paintComponent( Graphics g ) {
		...
		for ( int i = 0; i < shapes.size(); ++i ) {
			...
			shape.draw( gw, true, shape.currentlyHilited );
		}
		if ( is2ndPointBeingDraggedOut || is3rdPointBeingDraggedOut || isBoxSelection == 1) {
	...
	private static void usePointsSpecifiedByMouseToComputeShape(
		...
		s.color = color;
		switch ( currentTool ) {
			case SimplePaint.TOOL_SELECT_AND_MOVE:
				s = createRectange(s, p1,p2, x3,y3);
				break;
	...
	private static MyShape createRectange(MyShape s, Point2D p1, Point2D p2, float x3,float y3){
		s.type = MyShape.POLYGON;
		s.points.clear();
		s.points.add( p1 );
		s.points.add( new Point2D( p1.x(), p2.y()) );
		s.points.add( p2 );
		s.points.add( new Point2D( p2.x(), p1.y() ) );
		s.center.copy( Point2DUtil.computeCentroidOfPoints( s.points ) );
		float rotationAngle = Vector2D.computeSignedAngle(
			new Vector2D( p2.x()-s.center.x(), p2.y()-s.center.y() ),
	...
	private void updateHilitedShape() {
		...
		if( i > -1){
			if ( !shapes.get(i).currentlyHilited ) {
				shapes.get(i).currentlyHilited = true;
				repaint();
			}
			currentlyHilitedShape = i;
		}
		else if(currentlyHilitedShape > -1){
			shapes.get(currentlyHilitedShape).currentlyHilited = false;
			currentlyHilitedShape = -1;
			repaint();
		}
	}
	
	// 1 = true, 0 = false ,-1 = not on a shape
	private int checkOnAShapeHilited(){
		int i = indexOfShapeUnderPixel(mouse_x,mouse_y);
		if( i > -1){
			return (shapes.get(i).currentlyHilited?1:0);
		}
		return -1;
	}
	// set shape to hilited and check if a shape was in the rectangle's selection
	private boolean updateAllHilitedShapeSelected(MyShape rectSelection){
		boolean isShapeSelected = false;
		for(int i=0; i < shapes.size(); i++){
			if(checkIfInSelectionRectangle(rectSelection,shapes.get(i))){
				shapes.get(i).currentlyHilited = true;
	...
	private void resetSelection(){
		for(int i=0; i < shapes.size(); i++){
			if(shapes.get(i).currentlyHilited){
				shapes.get(i).currentlyHilited = false;
			}
		}
	}
	
	private boolean checkIfInSelectionRectangle(MyShape rectSelection, MyShape toCompare){
		ArrayList< Point2D > listPointCompare = toCompare.points;		
		for( int i=0; i<listPointCompare.size(); i++){
			if ( rectSelection.isPointInsideShape(
					gw.convertPixelsToWorldSpaceUnits( listPointCompare.get(i) )
				) ) {
						return true;
					}
		}
	...
	public void mouseReleased( MouseEvent e ) {
		...
		if ( simplePaint.currentTool == SimplePaint.TOOL_SELECT_AND_MOVE ) {
			if ( SwingUtilities.isLeftMouseButton(e) ) {
				...
				if(isBoxSelection == 1){
					if(updateAllHilitedShapeSelected(shapeBeingDraggedOut)){
						isBoxSelection++;
					}
					else{
						isBoxSelection = 0;
	...
	public void mouseDragged( MouseEvent e ) {
		...
		else if( isHilitedShapeBeingMoved && isBoxSelection > 0){
		  for (int i=0; i<shapes.size();i++){
			  MyShape shape = shapes.get(i);
			  if(shape.currentlyHilited == true){
				 shape.translate( Point2D.diff(
							gw.convertPixelsToWorldSpaceUnits( new Point2D( mouse_x, mouse_y ) ),
							gw.convertPixelsToWorldSpaceUnits( new Point2D( old_mouse_x, old_mouse_y ) )
				) ); 
